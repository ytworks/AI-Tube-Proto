...
class ApiService {
  ...
  async speechToText(audioBase64: string, format: string = 'webm'): Promise<SpeechToTextResponse> {
  ...
  async textToSpeech(text: string, voice?: string, speed?: number): Promise<TextToSpeechResponse> {
  ...
  async processVoice(audioBase64: string, format: string = 'webm', sessionId?: string): Promise<ProcessVoiceResponse> {
